...
class Set
{
  public:
    ...
    {
        createEmpty();
    }// Create an empty set (i.e., one whoe size() is 0).
...
template <typename ItemType> void unite(const Set<ItemType>& s1, const Set<ItemType>& s2, Set<ItemType>& result)
{
...
template <typename ItemType> void butNot(const Set<ItemType>& s1, const Set<ItemType>& s2, Set<ItemType>& result)
{
    {
...
template <typename ItemType> inline int Set<ItemType>::size() const
{
    return m_size;
}

template <typename ItemType> inline bool Set<ItemType>::empty() const
{
    return size() == 0;
}

template <typename ItemType> inline bool Set<ItemType>::contains(const ItemType& value) const
{
    Node* p = findFirstAtLeast(value);
